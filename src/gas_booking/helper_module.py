...
class Helper:
    def __init__(self, ACCESS_KEY, SECRET_KEY, REGION):
        ...
        self.DYN = boto3.client('dynamodb', region_name=REGION,
                                aws_access_key_id=ACCESS_KEY, aws_secret_access_key=SECRET_KEY)
        self.dynamoresource = boto3.resource(
            'dynamodb', region_name=REGION, aws_access_key_id=ACCESS_KEY, aws_secret_access_key=SECRET_KEY)
        self.SES = boto3.client(
            'ses', region_name=REGION, aws_access_key_id=ACCESS_KEY, aws_secret_access_key=SECRET_KEY)
        self.SQS = boto3.client(
            'sqs', region_name=REGION, aws_access_key_id=ACCESS_KEY, aws_secret_access_key=SECRET_KEY)
    
    # Adding new Entry to AWS DynamoDB
    def add_new_entry(self, table_name, primary_key_value, attributes):
        ...
        try:
            ...
            print(
                f'New entry added successfully with primary key {primary_key_value}.')
            return True
        except Exception as e:
            print(f'Error adding new entry: {e}')
            return False
        
     # Creating Table in AWS DynamoDB
    ...
    # Deleting records from AWS DynamoDB
    def delete_record(self, table_name, primary_key_value):
        ...
        try:
            ...
            print(
                f'Record with primary key {primary_key_value} deleted successfully.')
        except Exception as e:
            print(f'Error deleting record: {e}')

    # Approving Records in AWS DynamoDB
    def update_record(self, table_name, primary_key_value, update_attribute, new_value):
        ...
        try:
            ...
            print(
                f'Record with primary key {primary_key_value} updated successfully.')
        except Exception as e:
    ...
    def sqs_send_message_to_queue(self, queue_url, message_body):
        ...
        print(
            f"Message sent to queue '{queue_url}' with MessageId: {response['MessageId']}")
